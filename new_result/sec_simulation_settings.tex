...
segments) from open Seoul traffic data. The trace is a $15$-minute
live window ($89$ snapshots at $\Delta_{\mathrm{rd}}\approx10$\,s)
with $|\mathcal I|=180$ vehicles present throughout; since encoder
training converges over more rounds than one window, the $T=250$
rounds replay the window cyclically ($\approx2.8$ passes,
steady-state traffic), which keeps every per-round contact graph
real while no scheme's decision horizon spans a cycle ($H=6$,
version lifetime $15\ll89$ rounds). Vehicles communicate within
