...
We further validate the framework with \emph{real} multimodal data from two
autonomous-driving datasets, \textbf{KITTI} and \textbf{nuScenes}, both run over
the InTAS mobility. Each object is represented by its camera RGB patch and the
...
receiving strong encoders from others. Table~\ref{tab:main_results} and
Fig.~\ref{fig:real_accuracy} report the real classification accuracy (mean of 3
seeds). On both datasets the proposed framework attains the highest test
accuracy, and on KITTI the largest margin for poor-data vehicles, confirming on
real sensor data that demand- and mobility-aware encoder caching/forwarding
outperforms the caching and no-caching baselines.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{Figures/fig_infocom_accuracy.pdf}
\caption{Real multimodal FL: test accuracy vs.\ global round for (a) KITTI and
(b) nuScenes.}
