...
def _stats(res):
    ...
    for s in schemes:
        ...
        # learning utility U^learn of the paper's Table-I decomposition
        ulearn = _seed_curves(res[s], "utl").mean(1)
        rounds, cumtx = [], []
        ...
        out[s] = dict(
            ...
            ulearn=ulearn.mean(), ulearn_sd=ulearn.std(),
...
def make(out_path="new_result/tab_seoul_combined.tex",
         ...
         include_udel=False, include_ulearn=False, colsep="5pt"):
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    ncol = 7 + int(include_gap) + int(include_udel) + int(include_ulearn)
    lines = []
    a = lines.append
    ...
    if include_ulearn:
        # learning utility of the paper's utility decomposition (Sec. V)
        mid.append(r"$U^{\mathrm{learn}}$")
    if include_udel:
        # prose term of Fig. 3(d)/(h): "useful-delivery ratio"
    ...
    for tag, label in (("kitti", "KITTI"), ("nuscenes", "nuScenes")):
        ...
        best = dict(
            ...
            ulearn=max(st[s]["ulearn"] for s in schemes),
        ...
        for s in schemes:
            ...
            if include_ulearn:
                cells.append(_num(f"{d['ulearn']:.2f}",
                                  abs(d["ulearn"] - best["ulearn"]) < 1e-9))
            if include_udel:
...
if __name__ == "__main__":
    ...
    make("new_result/tab_seoul_combined_highdemand_udel.tex",
         ...
         include_gap=False, include_udel=True, include_ulearn=True,
         colsep="4.5pt")
